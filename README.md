...
What _IS_ clear, however, is that there is currently no Swarm-level scheduling happening, for say, if you had a multi-node cluster with specific nodes containing GPU's, based on [swarmkit #1244](https://github.com/docker/swarmkit/issues/1244), it sounds like there is no way to restrict/schedule a service/workload to this particular node. Also, GPUs don't really work in the same sense as CPU or Memory "shares", so not sure if this will ever be implemented. If your like me, and simply like using `single-node Docker Swarm` (because its cleaner and still has benefits over `docker-compose` imo) then this solution should suffice.
